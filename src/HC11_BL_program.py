...
def main(argv):
    ...
    ser = serial.Serial(
        port=comport, baudrate=1200, timeout=6.5
    )  # linux: /dev/tty/usb…  Windows: COMx
    ...
    while line:
        ...
        if line[0:2] == "S1":
            ...
            dcount = (
                bcount - 3
            )  # 2 bytes for address, 1 byte for checksum. Whatever is left is the number of bytes for data.
            i = 0
            j = int(line[4:8], 16)
            k = 0
            #            print (line)
            ...
            while k < (dcount):
                machine_code[j] = int(
                    line[i + 8 : i + 10], 16
                )  # not sure if int is automatically converted to byte!! <it does>
    ...
    byte = (
        ser.read()
    )  # HC11 will not echo back the 0xFF sync byte, but a serial loopback will.
    if not byte:
